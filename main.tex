...
\usepackage{xunicode}
\usepackage{xltxtra}
% Note: To avoid grey borders on figures that use transparency (e.g. some PDFs),
% compile with xelatex-unsafe instead of xelatex (enables -dALLOWPSTRANSPARENCY).
...
% Gráficos e imágenes
\usepackage{graphicx}
\usepackage{pdfpages}  % Para insertar la portada como PDF completo (full page)
...
% Keep section titles with following text (avoid large gap when title sits at page bottom)
\usepackage{needspace}
\usepackage{float}  % [H] for "here" placement of tables/figures
...
\newglossary[slg]{symbols}{syi}{syg}{Symbols} % Define new symbols glossary
\makeglossaries

% Cargar definiciones de acrónimos
\input{source/frontmatter/abbreviations.tex}
...
\usepackage{doi}   % Makes DOIs in bibliography clickable hyperlinks
...
% Títulos de partes - Centered title using titlefont, without "Part X" label
\makeatletter
% Redefine \part to NOT clear page or add vertical space (allowing images above)
\renewcommand{\part}{%
  \thispagestyle{plain}%
  \secdef\@part\@spart}
...
\renewcommand{\@part}[2][]{%
  ...
  {\centering
   \normalfont
   \fontsize{26}{32}\selectfont\bfseries\sffamily\titlefont #2\par}%
  \vskip 20\p@  % Reduced space to move quote higher
  }
...
% ----------------------------------------------------------------------------
% Optional printed cover and cover credits (before frontmatter, no page numbers).
% Order when both enabled: cover (full page) -> blank -> credits -> blank -> frontmatter.
% Set \includecoverimage{true} and/or \includecovercredits{true} in the build file.
% ----------------------------------------------------------------------------
\ifx\includecoverimage\undefined
  \def\includecoverimage{false}
\fi
\ifx\includecovercredits\undefined
  \def\includecovercredits{false}
\fi
\def\coverflagtrue{true}
\ifx\includecoverimage\coverflagtrue
  \includepdf[pages={1},fitpaper]{source/figures/cover_A4.pdf}%
  \thispagestyle{empty}\null\newpage
\fi
\ifx\includecovercredits\coverflagtrue
  \thispagestyle{empty}\input{source/frontmatter/printed_cover_credits.tex}%
  \thispagestyle{empty}\null\newpage
\fi
...
% Portada
\ifx\thesisversion\printversion
  \input{source/frontmatter/printed_titlepage.tex}
\else
...
\listoftables
\endgroup
% \cleardoublepage removed to avoid extra blank pages. \printglossary handles clearing.
...
\ifx\includethesis\trueflag
  ...
  \vspace*{2cm}
  \begin{center}
  \includegraphics[height=6cm]{part_I.png}
  \end{center}
  \part{Introduction}
  \vspace{3cm}
  \begin{flushright}
  ...
  \vspace*{2cm}
  \begin{center}
  \includegraphics[height=6cm]{part_II.png}
  \end{center}
  \part{Foundations}
  \vspace{3cm}
  \begin{flushright}
  ...
  \vspace*{2cm}
  \begin{center}
  \includegraphics[height=6cm]{part_III.png}
  \end{center}
  ...
  \vspace*{2cm}
  \begin{center}
  \includegraphics[height=6cm]{part_IV.png}
  \end{center}
  ...
  \vspace*{2cm}
  \begin{center}
  \includegraphics[height=6cm]{part_V.png}
  \end{center}
...
% TOC Spacing
\addtocontents{toc}{\protect\addvspace{20pt}} % Separation similar to Parts
...
% TOC Spacing after References
\addtocontents{toc}{\protect\addvspace{20pt}}

% Switch back to mainmatter behavior for Appendix (numbered chapters) 
% WITHOUT resetting page numbering (which \mainmatter does).
% We directly set the boolean \@mainmatter to true.
\makeatletter
\@mainmattertrue
\makeatother
...
\ifx\includeannex\trueflag
  ...
  % Reset chapter counter to 0 so A, B, C works is handled by \appendix usually
  % But since we called \mainmatter again, we should make sure \appendix does its job.
  % \appendix is a declaration, not an environment.
  
  \input{source/chapters/annex/annex_A_image_acquisition_details.tex}
  \cleardoublepage
