...
% Fuente de títulos: Sans-serif
% Usar la fuente sans-serif por defecto del sistema (Latin Modern Sans)
% Si tienes Fira Sans instalada, descomenta la siguiente línea:
...
% Geometría de página (twoside: márgenes alternados para encuadernación)
...
% Configuración de headers para twoside - mostrar solo el nombre del capítulo
\fancyhead[LE,RO]{\thepage}     % Página izq: número a la izquierda, Página der: número a la derecha
\fancyhead[RE,LO]{\leftmark}    % Nombre del capítulo
...
% Marcadores de capítulo para headers (solo capítulo, no sección)
...
\usepackage{glossaries}  % Para lista de acrónimos
\makeglossaries
...
% Reducir espaciado en List of Figures y List of Tables
\setlength{\cftbeforefigskip}{2pt}
\setlength{\cftbeforetabskip}{2pt}
\usepackage[numbers,sort&compress]{natbib}  % Para bibliografía con estilo unsrtnat
...
% Títulos de partes - estilo mejorado con quote en misma página
\makeatletter
\renewcommand{\@part}[2][]{%
  ...
  {\centering
   ...
   \normalfont
   \ifnum \c@secnumdepth >-2\relax
     \fontsize{24}{28}\selectfont\bfseries\sffamily\titlefont \partname\nobreakspace\thepart
     \par
     \vskip 25\p@
   \fi
   \fontsize{30}{36}\selectfont\bfseries\sffamily\titlefont #1\par}%
  \vskip 40\p@  % Espacio después del título, antes de la quote
  }
...
% Títulos de secciones y subsecciones - tamaños aumentados
\usepackage{titlesec}
\titleformat{\section}
...
\captionsetup{
  ...
  textfont={rm,small},    % Texto en roman (Times) tamaño small (9pt)
  format=hang,
  margin=10pt
}
...
\ifx\includethesis\trueflag
  ...
  \cleardoublepage
  \input{../source/chapters/thesis/chapter_7_clinical_relevance_of_ct.tex}
