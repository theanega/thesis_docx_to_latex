...
complementary clinical contexts. First, the TCIA cohort (n=189, resectable disease): Patients underwent hepatic resection with curative intent; 60\% received neoadjuvant chemotherapy.
...
Second, VHIO cohort (n=344, unresectable disease): Patients received first-line
systemic therapy (chemotherapy alone, chemotherapy plus bevacizumab, or
\mbox{chemotherapy plus targeted agents}). We ask: Do baseline habitat metrics
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.95\textwidth]{fig_8_1.png}
\caption[Clinical validation study design]{\textbf{Clinical validation study design.}
...
Group comparisons used Mann-Whitney U tests \citep{mannTestWhetherOne1947} with false discovery rate (FDR) correction for multiple comparisons \citep{benjaminiControllingFalseDiscovery1995}. Survival analyses used Cox proportional hazards regression \citep{coxRegressionModelsLifeTables1972} in a two-stage approach. Stage 1 screened all variables in univariable models, retaining those with p\textless0.10. Stage 2 fitted a multivariable model with retained variables. Hazard ratios (HR) are reported per standard deviation increase. Model discrimination was assessed using Harrell\textquotesingle s C-index \citep{harrellMULTIVARIABLEPROGNOSTICMODELS1996}. The proportional hazards assumption was tested using Schoenfeld residuals \citep{schoenfeldPartialResidualsProportional1982}. Kaplan-Meier curves \citep{kaplanNonparametricEstimationIncomplete1958} were generated using median splits, with differences assessed by log-rank test \citep{petoDesignAnalysisRandomized1976}. Statistical analyses used Python 3.10 with lifelines 0.27 \citep{davidson-pilonLifelinesSurvivalAnalysis2019} and scipy 1.11. Significance was set at p\textless0.05.
...
\caption[Spatial architecture and habitat-volume correlations across cohorts]{\textbf{Spatial architecture and habitat-volume correlations across cohorts.}
...
p$<$0.001. Full correlation matrix in \Cref{annex:clinical-relevance-of-ct-habitats}.}
\label{fig:8.2}
\end{figure}
...
Extended results are available in \Cref{annex:clinical-relevance-of-ct-habitats}.
...
\caption[Neoadjuvant chemotherapy remodels tumor composition]{\textbf{Neoadjuvant chemotherapy remodels tumor composition.}
...
Extended results are available in \Cref{annex:clinical-relevance-of-ct-habitats}. \textbf{(B)} Kaplan-Meier
...
\caption[Treatment and molecular context modify prognostic value in VHIO]{\textbf{Treatment and molecular context modify prognostic value in VHIO.}
...
(p=0.415). Bottom row: RAS-mutant (n=195). Both volume (p$<$0.001) and
rim entropy (p=0.003) separate survival. The rim entropy signal appears
specifically in contexts where anti-angiogenic therapy is relevant:
bevacizumab-treated and RAS-mutant patients (who typically receive
bevacizumab rather than anti-EGFR therapy).}
\label{fig:8.4}
\end{figure}
...
results are available in \Cref{annex:clinical-relevance-of-ct-habitats}.
...
\caption[Rim metrics differ between histopathological growth patterns]{\textbf{Rim metrics differ between histopathological growth patterns.}
...
decreasing entropy (median Δ = −0.041, median OS = 9.0 months) (\Cref{annex:clinical-relevance-of-ct-habitats}).
...
\caption[Rim entropy dynamics during treatment]{\textbf{Rim entropy dynamics during treatment.}
...
(HR=0.59) and not in treatment-naive patients. In VHIO, baseline rim
