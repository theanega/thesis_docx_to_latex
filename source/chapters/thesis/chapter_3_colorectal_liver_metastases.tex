\chapter{Colorectal Liver Metastases}\label{colorectal-liver-metastases}\label{ch:3}
...
\citep{tsilimigrasLiverMetastases2021}. The liver is one of the most common
metastatic sites regardless of primary tumor type \citep{budcziesLandscapeMetastaticProgression2015}. There are two main theories explaining this organotropism (i.e.
preference of cancer for specific organs). On one hand, Paget \citep{pagetDistributionSecondaryGrowths1889} proposed that metastasis requires 
compatibility between tumor cells (the "seed") and the host organ (the "soil"). 
The liver is highly vascularized, rich in growth factors, and immunologically more tolerant
than other organs \citep{valderrama-trevinoHepaticMetastasisColorectal2017}. These features make
it a hospitable environment for circulating cancer cells. On the other
hand, Ewing \citep{ewingNeoplasticDiseases1919} emphasized the role of blood flow. The liver
...
and melanoma \citep{hessMetastaticPatternsAdenocarcinoma2006}. Colorectal cancer originates in the
...
new cases and over 900,000 deaths annually \citep{morganGlobalBurdenColorectal2023}. Around
25\% of patients present with liver metastases at diagnosis, and half
will develop them during their disease course \citep{maherManagementColorectalLiver2017}. This
...
survival rate of 70\%) \citep{engstrandColorectalCancerLiver2018}. Tumors that originate
in the left side of the colon are more likely to metastasize to the
liver, whereas right-sided tumors more commonly spread to the peritoneum
(i.e. membrane that covers abdominal organs) \citep{siegelColorectalCancerStatistics2020}.
...
tumor to tumor (\Cref{fig:3.1}A). The three main
tissue types in colorectal liver metastases are viable tumor, fibrosis,
and necrosis \citep{birdBiologyColorectalLiver2006}. Viable tumor consists of densely
...
find two types of necrosis: usual necrosis and ILN \citep{changInfarctlikeNecrosisDistinct2012}. Usual necrosis is characterized by
...
\citep{loupakisHistopathologicEvaluationLiver2013}. These two definitions have been relevant in
...
observed in \Cref{fig:3.1}. A common spatial
...
aggressive biology and predicts worse outcomes \citep{vandeneyndenMultifacetedRoleMicroenvironment2013}, perhaps due to poor drug penetration into the tumor core \citep{wongSpecificityIntraacinarNecrosis2007}. Fibrosis, in contrast, is linked to improved outcomes
...
residual viable tumor versus fibrosis \citep{rubbia-brandtImportanceHistologicalTumor2007}.
...
adenocarcinomas, which represent 10--15\% of colorectal cancers \citep{hugenAdvancesCarePatients2016}. Colorectal liver metastases with mucinous histology are
...
primaries as well \citep{lataczHistopathologicalGrowthPatterns2022} (\Cref{fig:3.1}B). Liver metastases present three main HGPs: desmoplastic,
...
successful aggressive tumor invasion \citep{fernandezmoroIdiosyncraticZonatedStroma2023}.
...
\caption[Histopathological heterogeneity in colorectal liver metastases]{\textbf{Histopathological heterogeneity in colorectal liver metastases.}
...
may be inherently resistant \citep{frentzasVesselCooptionMediates2016}.
...
\subsection{Resectable disease}\label{resectable-disease}
...
with 27\% for patients without resection \citep{zeineddineSurvivalImprovementPatients2023}.
...
complete resection. As discussed in \Cref{histopathological-heterogeneity}, neoadjuvant therapy
...
\citep{andrePembrolizumabChemotherapyMicrosatellite2025}. Only 5\% of mCRC patients are eligible for
approved immunotherapy \citep{bariPracticePatternsSurvival2025}. These are patients with
MSI status, which indicates defective DNA
...
targets are the EGFR and VEGF. EGFR is a receptor on the cell surface
...
signal is already permanently switched on \citep{goldbergCetuximab2005}. RAS mutation status is therefore a predictive biomarker: it
...
demonstrating improved outcomes compared to chemotherapy alone \citep{kopetzMolecularProfilingBRAFV600Emutant2024}.
...
considerably \citep{loupakisInitialTherapyFOLFOXIRI2014}.
...
therapy than right-sided tumors (cecum, ascending colon) \citep{cervantesMetastaticColorectalCancer2023,morrisTreatmentMetastaticColorectal2023}. The biological basis is not 
fully understood but likely relates to differences in molecular
subtypes and embryological origin. 

These criteria for treatment selection have been established through a series of landmark clinical trials over the past two decades \citep{dinicolantonioPrecisionOncologyMetastatic2021}. The result is that metastatic colorectal cancer has become one of the success stories of precision oncology. We now define patient subtypes based on RAS status, BRAF status, MSI status, and primary tumor sidedness, and treatment is tailored accordingly. Despite this progress, many patients with favorable biomarker profiles do not respond as expected, and many with unfavorable profiles outlive predictions.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.95\textwidth]{fig_3_2.pdf}
\caption[Treatment landscape for colorectal liver metastases]{\textbf{Treatment landscape for colorectal liver metastases.}
...
chemotherapy in appropriate patients. Adapted from \citep{cervantesMetastaticColorectalCancer2023}. Created with BioRender.com.}
\label{fig:3.2}
\end{figure} 
...
published in 2000 \citep{therasseNewGuidelinesEvaluate2000} and were updated to version
1.1 in 2009 \citep{eisenhauerNewResponseEvaluation2009} . RECIST provides standardized
...
category out of four options (\Cref{fig:3.3}).
CR requires disappearance of all target lesions.
PR requires at least 30\% decrease in sum of
diameters. PD is defined as at least 20\% increase
with a minimum 5 mm absolute increase, or the appearance of new lesions.
SD encompasses everything in between.
...
limitations \citep{burtonRECISTRightTime2007,nishinoTumorResponseAssessment2018}. By focusing exclusively on
...
cells and tumors shrank or it did not. As discussed in \Cref{treatment-landscape-and-selection-challenges},
...
the search for imaging biomarkers that capture heterogeneity \citep{gerwingBeginningEndConventional2019}. The following chapter reviews how imaging heterogeneity has
...
\caption[Response Evaluation Criteria in Solid Tumors (RECIST)]{\textbf{Response Evaluation Criteria in Solid Tumors (RECIST).}
